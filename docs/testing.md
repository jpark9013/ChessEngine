...
CI job `strength` plays our engine against **Stockfish 17.1** with `UCI_LimitStrength` and `UCI_Elo=2200`. All eight games are **60+0** (1+0 sudden death: 60 seconds **whole-game** clock per side, no increment). Stockfish is given remaining `wtime`/`btime` so its own time manager runs — not a 100ms `movetime`. Our engine uses `allocate_time(..., mode="live")` against the same remaining clock (about **1.5s** opening / **8s** hard on a full 60s clock). Up to four games run at a time. Deploy requires **≥ 4 points** over 8 games (win = 1, draw = 0.5, loss = 0); exactly 4.0 passes. The match stops early if the floor is already met or can no longer be reached, cancelling games that have not started. A side whose remaining clock hits 0 loses on time. Any crash or illegal move fails the job immediately.

```bash
uv sync --group strength
PYTHONPATH=build:bot uv run python scripts/gauntlet.py \
  --stockfish /path/to/stockfish \
  --games 8 --elo 2200 --min-points 4 --clocks 60+0 --concurrency 4 \
  --pgn gauntlet.pgn
```
...
`--clocks` is a comma-separated list cycled by game index. Bare integers are **seconds** (so `60` is 60s sudden death, `3+2` is 3s+2s increment, not 3 minutes). Minutes need a suffix: `5m+0`, `5min+0`, `1m+0`, `1m30s+2`. Increment is Fischer (added after a completed move). Both sides spend from the remaining game clock: we use the live Stockfish-style allocator (remaining clock + ~0.65× increment); Stockfish uses its own.

## Elo estimate (after deploy)

CI job `estimate-elo` runs only after **`strength` and `deploy` both succeed** (`needs: [strength, deploy]`), so it is main-push-only. It binary-searches Stockfish `UCI_Elo` with the same 8-game protocol as the strength gate (default `--clocks 60+0`, live remaining-clock protocol — Stockfish gets `wtime`/`btime`, not 100ms/move). Each probe is a match vs `UCI_Elo=mid`; ≥ 4.0 points means we are at least `mid` (`lo = mid`), else `hi = mid`. It stops when `hi - lo <= 100` and reports `(lo+hi)//2`, i.e. **±50**. Eight games are noisy; the number is a CI snapshot, not a 400-game rating.

Probes are **sequential** (each needs the previous `lo`/`hi`). Only the games *inside* one probe run in parallel.

`--concurrency` is **1–4**. Strength and calibrate both use **4** in CI. A GitHub `ubuntu-latest` runner has about 4 vCPU; each game is this engine (single-thread search, ~16 MB TT) plus 1-thread Stockfish (16 MB hash). Four pairs is ~8 threads on 4 cores — roughly 2× oversubscription, around 50–100 Elo of NPS loss, which is the budget. More than 4 would exceed that, so the CLI rejects it. Eight 1+0 games with both sides using the clock take minutes, not seconds; the strength job allows 60 minutes and calibrate 90.

```bash
PYTHONPATH=build:bot uv run python scripts/gauntlet.py \
  --calibrate --stockfish /path/to/stockfish \
  --games 8 --min-points 4 --clocks 60+0 --concurrency 4 \
